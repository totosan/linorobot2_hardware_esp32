...
void loop()
{
    ...
    if (current_motor == 0 && tk % run_time == run_time - 1)
        {max_rpm = current_rpm1;}
    if (current_motor == 1 && tk % run_time == 0)
        {stopping = current_rpm1;}
    if (current_motor == 1 && tk % run_time == run_time - 1)
        {max_rpm = current_rpm2;}
    if (total_motors == 2 && current_motor == 0 && tk % run_time == 0)
        {stopping = current_rpm2;}
    if (current_motor == 2 && tk % run_time == 0)
        {stopping = current_rpm2;}
    if (current_motor == 2 && tk % run_time == run_time - 1)
        {max_rpm = current_rpm3;}
    if (current_motor == 3 && tk % run_time == 0)
        {stopping = current_rpm3;}
    if (current_motor == 3 && tk % run_time == run_time - 1)
        {max_rpm = current_rpm4;}
    if (total_motors == 4 && current_motor == 0 && tk % run_time == 0)
        {stopping = current_rpm4;}
    ...
    if (tk && tk % run_time == 0)
    {
        ...
        printf("MOTOR%d SPEED %6.2f m/s %6.2f rad/s STOP %6.3f m \n max_rpm: %4.3f , stopping: %4.2f (x: %6.3f y: %6.3f z: %6.3f)\n", current_motor ? current_motor : total_motors,
               max_linear.linear_x, max_angular.angular_z, max_linear.linear_x * stopping / max_rpm, max_rpm, stopping, max_linear.linear_x, max_linear.linear_y, max_linear.angular_z);
